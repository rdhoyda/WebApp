...
class Product {
    constructor(inputString) {
      ...
      this.name = name;
      this.price = parseFloat(priceStr); // Parse the price as a floating-point number
    ...
    static printProducts = (products) => {
        ...
        for(let i=0;i<products.length;i++){
            if(products[i].availability === 'In Stock'){
                console.log(`Product: ${products[i].name}, Cost: ${new Intl.NumberFormat('us-US', {style: 'currency', currencyDisplay: "narrowSymbol", currency: 'USD'}).format(products[i].price)}, Availability: ${available}`);
            }
            else{
                console.log(`Product: ${products[i].name}, Cost: ${new Intl.NumberFormat('us-US', {style: 'currency', currencyDisplay: "narrowSymbol",  currency: 'USD'}).format(products[i].price)}, Availability: ${notAvailable}`);
            }
        }
        //debug
